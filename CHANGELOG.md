...
- A `/frequency` page: how often each Qur'anic root appears, in five tiers from
  ...
  hover. Reached from a "Did you know" line on the landing page. Descriptive by
  design — it states what the counts are and stops there. The page is a Server
  Component: its 138 KB of aggregates stay on the server, and the Top-N / page
  control is CSS-only, so the table costs no client JavaScript.
- `docs/ROOT-GLOSS-GAPS.md`, generated by `npm run data:root-dist`: the roots still
  missing an English gloss, ranked by share of the text. 1,462 of 1,642 roots
...
- The canvas behind every page. Three fixed layers each painted accent radials on
  ...
  workspace atmosphere — and in the light theme they mixed over the warm
  parchment base into an olive-khaki wash, with a glow anchored below the
  viewport smearing across the bottom edge of every page. The canvas is now flat:
  `--bg-0` in dark, a plain `--bg-0 → --bg-1` in light.
